...
namespace BLL.FlightPlan
{
    public class SuperMapBLL
    {
        ...
        private DAL.FlightPlan.CurrentFlightPlanDAL currentdal = new DAL.FlightPlan.CurrentFlightPlanDAL();
        private DAL.FlightPlan.FileCurrentPlanMasterDAL currentmasterdal = new DAL.FlightPlan.FileCurrentPlanMasterDAL();
        ...
        public List<ViewModel.FlightPlan.SuperMapVM> GetCurrentMapData(string planID)
        {
            var superMap = new List<ViewModel.FlightPlan.SuperMapVM>();
            var current = currentdal.GetData(planID);
            var masters = new List<Model.EF.File_Master>();
            if(string.IsNullOrEmpty(current.FlightPlanID))
                masters = masterdal.GetByMasterID(currentmasterdal.GetMasterList(planID));
            else
                masters = masterdal.GetByMasterID(flightplandal.GetMasterList(planID));
            foreach (var master in masters)
            {
                var map = new ViewModel.FlightPlan.SuperMapVM();
                map.MasterID = master.ID;
