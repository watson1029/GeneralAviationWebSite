...
(function (j) {
    ...
    //跨框架传递数据
    dj.data = function (name, value) {
        var top = dj.top, cache = top[dataName] || {};
        top[dataName] = cache;
        if (value != undefined) {
            cache[name] = value;
        } else {
    ...
    dj.removeData = function (name) {
        try {
            var cache = dj.top[dataName];
            if (cache && cache[name]) delete cache[name];
        } catch (e) { }
    };
    dj.clearData = function () {
        if (dj.top[dataName]) { delete dj.top[dataName]; }
    };
    //    j(window).bind("unload", dj.clearData);
    //iframe操作
    dj.iframe = {
        //自适应高度和宽度
        auto: function (id) {
            var frame = dj.dom(id);
            dj.loaded(id, function () {
                dj.iframe.setHeight(this);
            });
        },
        setHeight: function (frame) {
            frame = typeof (frame) == "string" ? dj.dom(frame) : frame;
            var pos = dj.position.maxClient(frame.contentWindow);
            frame.height = pos.height + "px";
